class Venue
  ...
  def self.search(param_search)
    ...
    if param_search[:lat]
      ...
      return Venue.near([param_search[:lat].to_f, param_search[:lng].to_f], param_search[:distance].to_f, order:"distance")
    end

    if param_search[:search_query]
      result = Venue.near(param_search[:search_query], 2, order:"distance").to_a + Venue.any_of(
