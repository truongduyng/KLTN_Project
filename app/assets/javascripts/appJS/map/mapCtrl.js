app.controller('mapCtrl', ['$scope', '$http', 'Auth', function($scope, $http, Auth){
  ...
  function SearchBox() {
    var searchTextBox = $('<form><input type="text" id="GeoSearch" class="form-control" placeholder = "Tìm kiếm địa chỉ" style= "width: 250px; margin-bottom: 5px; font-size: 14px;" ng-model = "query_address" ng-submit= "search_adddress()"/></form>');
    var div = $('<div class="geoSearchBox" ng-controller = "mapCtrl"></div>').append(searchTextBox);
    return div.get(0);
  }
    ...
    geocoder.geocode({'latLng': map.getCenter()}, function(results, status) {
     if (status == google.maps.GeocoderStatus.OK) {
      ...
      if (results[0]){
        ...
        markers = [];
        $http.get("/search/"+Math.floor(results[0].geometry.location.k)+"/"+Math.floor(results[0].geometry.location.D)).success(function(data){
          setMarkers(map,data);
        });
      }
    }else{
